...
def plot_curve(df, x_col_plot, y_col_plot, x_col_base, y_col_base,
    ...
    Args:
        x_col_plot (str): Column name for the X-axis visual plot (can be in converted units).
        y_col_plot (str): Column name for the Y-axis visual plot (can be in converted units).
        x_col_base (str): Column name for the X-axis linear fit (ALWAYS in standard units).
        y_col_base (str): Column name for the Y-axis linear fit (ALWAYS in standard units).
        y_base_units (str): The standard unit of the y_col_base (e.g., 'MPa').
    """
    ...
    if fit_line:
        ...
        if not fit_df.empty and len(fit_df) > 1:
            ...
            print(f"Fit Region:       {fit_region_text}")
            print(f"Calculated Modulus: {modulus_val:.3f} {modulus_units}")
            print(f"Y-Intercept (Stress-Axis):  {y_intercept:.4f} {y_base_units}")
            print(f"X-Intercept (Strain-Axis):  {x_intercept:.5f} (unitless)")
            print("-------------------------------------------------")
    ...
    plt.close()
    print(f"Static plot and analysis saved to: {os.path.basename(output_path)}")
...
def animate_curve(df, x_col, y_col, title, x_label, y_label, output_path,
    ...
    if total_data_points < 2:
        print(f"Warning: Not enough data (< 2 points) to create animation for '{title}'. Skipping.")
        return
    ...
    if ideal_frame_skip < 1:
        ...
        print(f"Warning: Not enough data for a full {target_duration_s}s video at {target_fps} FPS.")
        print(f"         Using every data point (frame_skip=1). Video will be shorter.")
    else:
        frame_skip = max(1, round(ideal_frame_skip))
        num_frames_to_render = total_data_points // frame_skip

    print(f"\nCreating animation:")
    print(f"  - Target: {target_duration_s}s at {target_fps} FPS ({total_frames_target} target frames).")
    print(f"  - Data: {total_data_points} points available.")
    print(f"  - Calculated frame_skip: {frame_skip} (to produce {num_frames_to_render} frames).")
    ...
    ani = animation.FuncAnimation(fig, update, frames=num_frames_to_render, init_func=init, blit=True, interval=20)

    print(f"Saving animation to: {os.path.basename(output_path)} (this may take a moment)")
